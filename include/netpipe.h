 ...
 *
 * Public API Surface Audit & Freeze (v0.1.0):
 * All opaque struct layouts (np_pipeline_t, np_source_t, np_filter_t,
 * np_processor_t, np_sink_t) are hidden internally to ensure ABI stability.
 * Exposed struct layouts (like np_packet_t) contain reserved padding
 * to support backward-compatible future extensions.
 */
...
/* Annotation for tentative or experimental APIs subject to change */
...
typedef struct np_packet {
    ...
    /* User-defined metadata context */
    void            *user_data;

    /* Reserved padding for future ABI-stable expansions */
    void            *reserved[4];
} np_packet_t;
...
NP_EXPERIMENTAL np_sink_t *np_sink_tuntap(const char *uri);        /* TUN/TAP inject sink    */
NP_EXPERIMENTAL np_sink_t *np_sink_socket(const char *uri);        /* Remote socket sink     */
