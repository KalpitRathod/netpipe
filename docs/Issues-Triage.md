...
|---|-------------|----------------|-------------|
| 1 | [TCP Stream Reassembly Lacks Out-of-Order Segment Buffering](#1-tcp-stream-reassembly-lacks-out-of-order-segment-buffering) **(RESOLVED in 0.1.0)** | `src/processor/np_tcp_stream.c` | `known-limitation` |
...
* **Resolution**:
  The reassembler now implements a per-flow, per-direction sorted segment
  queue with overlap clipping, hole-timeout flushing (1s default),
  SYN/FIN/RST state machine, RFC 793 sequence arithmetic, and re-SYN
  handling.  9 unit tests + 18 real-world stress tests in
  `tests/test_tcp_reassembly.c` and `tests/test_tcp_reassembly_stress.c`
  verify all the scenarios the original issue called out.  See
  `RELEASE_NOTES.md` §1 for details.
