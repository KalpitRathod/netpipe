...
REPO = pathlib.Path(os.environ.get("NETPIPE_REPO_ROOT", "")).resolve() if os.environ.get("NETPIPE_REPO_ROOT") else pathlib.Path(__file__).resolve().parent.parent.parent
...
def main():
    if not PCAP.exists() or not KEYLOG.exists():
        ...
        print(f"  Run: python3 tests/python/gen_tls_fixtures.py")
        sys.exit(1)
