                            ...
                            const uint8_t *data, size_t len)
{
    ...
    /* skip 802.1Q and QinQ (802.1ad) VLAN tags
     * 802.1Q  (0x8100): single-tagged — common on enterprise/DC networks
     * 802.1ad (0x88A8): outer tag of double-tagged (QinQ) frames;
     *                   followed by an inner 0x8100 tag which we also skip
     *
     * Bug 6 fix: track the total Ethernet header length (including VLAN
     * tags) so we can push the ETH layer with the correct length.  The
     * old code always pushed 14 bytes, which was wrong for 802.1Q
     * (18 bytes) and QinQ (22 bytes) tagged frames.  Downstream code
     * that uses pkt->eth->len to locate the payload was broken. */
    size_t eth_hdr_total = sizeof(eth_hdr_t);  /* 14 bytes base */
    ...
    /* Bug 6 fix: push the ETH layer with the ACTUAL header length
     * (14 + 4 per VLAN tag), not the fixed 14-byte base. */
    np_layer_t *l = np_packet_push_layer(pkt, NP_PROTO_ETH, data, eth_hdr_total);
...
{
    /* Bug 1.1: the old heuristic only checked the first 3 bytes (content
     * type + version major/minor), which false-positives on any binary
     * stream that starts with e.g. 0x17 0x03 0x03.  Now we also validate
     * the 2-byte record length field: it must fit within the remaining
     * payload and must not exceed the TLS maximum (16384 + 2048 for
     * ciphertext expansion per RFC 8446 §5.1).  For handshake records
     * (type 22), we additionally require the first handshake byte to be
     * a known type (1=ClientHello, 2=ServerHello, 11=Certificate, etc.). */
    if (len < 5) return false;
    uint8_t ct = payload[0];
    uint8_t maj = payload[1], min = payload[2];
    if (!(ct >= 20 && ct <= 23) || maj != 3 || min > 4) return false;

    /* Validate record length. */
    uint16_t rlen = ((uint16_t)payload[3] << 8) | payload[4];
    if (5u + rlen > len) return false;       /* record claims more bytes than available */
    if (rlen > 16384 + 2048) return false;   /* exceeds TLS max ciphertext size */

    /* For handshake records, validate the handshake type byte. */
    if (ct == 22 && rlen >= 1) {
        uint8_t hs_type = payload[5];
        /* Known TLS handshake types (RFC 5246 §7.4, RFC 8446 §4.2). */
        static const uint8_t known_hs_types[] = {
            0,  /* HelloRequest */
            1,  /* ClientHello */
            2,  /* ServerHello */
            4,  /* NewSessionTicket (TLS 1.2) */
            5,  /* EndOfEarlyData (TLS 1.3) */
        ...
        bool hs_ok = false;
        for (size_t i = 0; i < sizeof(known_hs_types); i++) {
            if (hs_type == known_hs_types[i]) { hs_ok = true; break; }
        }
                           ...
                           np_proto_t transport_proto)
{
    ...
    /* Try the extra protocol decoders (DHCP, QUIC, SIP, MQTT, VXLAN). */
    np_demux_decode_app_extra(pkt, payload, len, src_port, dst_port,
                               transport_proto);
    return NP_OK;
}
...
/*                                                                      */
/*  RFC 2460 / RFC 8200: Next Header values that indicate an extension  */
/*  header rather than a transport payload:                             */
/*    0  = Hop-by-Hop Options                                           */
/*   43  = Routing Header                                               */
/*   44  = Fragment Header                                              */
/*   51  = Authentication Header (AH)  — fixed 8-byte minimum          */
/*   60  = Destination Options                                          */
/*  135  = Mobility Header                                              */
/*  139  = HIP                                                          */
/*  140  = Shim6                                                        */
/*                                                                      */
/*  Each extension header (except Fragment and AH) starts with:         */
/*    byte 0: Next Header                                               */
/*    byte 1: Header Extension Length in 8-octet units, not counting   */
/*            the first 8 octets.                                       */
/*  Fragment header is exactly 8 bytes.                                 */
/*  AH header: bytes 1 is payload length in 4-octet words minus 2.     */
/*                                                                      */
/*  Returns false and sets *proto_out / *data_out / *len_out to point   */
/*  at the transport payload, or returns false on truncation.           */
/* ------------------------------------------------------------------ */

/* Extension-header Next Header values (RFC 8200 §4) */
#define IPV6_EH_HOP_BY_HOP  0
#define IPV6_EH_ROUTING     43
#define IPV6_EH_FRAGMENT    44
#define IPV6_EH_AH          51
#define IPV6_EH_DEST_OPTS   60
...
{
    return nh == IPV6_EH_HOP_BY_HOP ||
           nh == IPV6_EH_ROUTING    ||
           nh == IPV6_EH_FRAGMENT   ||
           nh == IPV6_EH_AH        ||
           nh == IPV6_EH_DEST_OPTS  ||
           nh == IPV6_EH_MOBILITY   ||
           nh == IPV6_EH_HIP        ||
           nh == IPV6_EH_SHIM6;
}
...
/*
 * Walk the IPv6 extension-header chain starting immediately after the
 * fixed 40-byte IPv6 header.  On success writes the final transport
 * protocol, the pointer to the first transport-layer byte, and the
 * remaining length into *proto, *payload, *paylen, then returns true.
 * Returns false on truncation or a malformed extension header.
 */
static bool ipv6_skip_ext_headers(uint8_t        initial_nh,
                                  const uint8_t *data,
                                  size_t         len,
                                  uint8_t       *proto,
                                  ...
                                  size_t        *paylen)
{
    ...
    while (is_ipv6_ext_header(nh)) {
        if (rem < 8) return false;     /* all ext-hdrs are ≥ 8 bytes */

        size_t hdr_len;
        if (nh == IPV6_EH_FRAGMENT) {
            /* Fragment header is always exactly 8 bytes */
            hdr_len = 8;
        } else if (nh == IPV6_EH_AH) {
            /* AH: len field is in 4-octet units, value = (hdr_octets/4) - 2 */
            hdr_len = ((size_t)p[1] + 2) * 4;
        } else {
            /* All others: len field is in 8-octet units, not counting first 8 */
            hdr_len = ((size_t)p[1] + 1) * 8;
        }

        if (hdr_len < 8 || hdr_len > rem) return false;
...
np_err_t np_demux_packet(np_packet_t *pkt, np_linktype_t linktype)
{
    ...
    case NP_LINK_ETHERNET: {
        ...
        if (et == 0x0800) {
            ...
            if (after_len >= sizeof(ip6_hdr_t)) {
                ...
                if (ipv6_skip_ext_headers(ip6->next_header, xdata, xlen,
                                          &ip_proto, &net_data, &net_len)) {
                    /* net_data/net_len/ip_proto already set by walker */
                }
            }
        }
        break;
    ...
    case NP_LINK_LINUX_SLL: {
        ...
        /* Bug 5 fix: use memcpy instead of unaligned uint16_t deref. */
        uint16_t et_v;
        memcpy(&et_v, data + 14, sizeof(et_v));
        uint16_t et = ntohs(et_v);
    ...
    switch (ip_proto) {
    case IPPROTO_TCP: {
        /* Bug 6 fix: check the return value of decode_tcp.  If the TCP
         * header is malformed (hdrlen < 20 or hdrlen > len), decode_tcp
         * returns NP_ERR_PROTO and does NOT push the transport layer.
         * The old code ignored the return value and unconditionally
         * extracted ports and advanced net_data using a bogus hdrlen,
         * leading to garbage app-layer decode and a violated invariant
         * (pkt->app set while pkt->transport is NULL). */
        if (decode_tcp(pkt, net_data, net_len) != NP_OK) {
            return NP_OK;  /* malformed TCP header — skip app decode */
        }
        if (net_len >= sizeof(tcp_hdr_t)) {
            ...
            uint8_t hdrlen = ((tcp->data_offset_flags >> 4) & 0x0f) * 4;
            /* Validate hdrlen before advancing (decode_tcp already
             * checked, but defense-in-depth). */
            if (hdrlen < 20 || hdrlen > net_len) {
                return NP_OK;
            }
            net_data += hdrlen;
    ...
    case IPPROTO_UDP: {
        /* Bug 6 fix: same as TCP — check decode_udp return value. */
        if (decode_udp(pkt, net_data, net_len) != NP_OK) {
            return NP_OK;  /* malformed UDP header — skip app decode */
        }
