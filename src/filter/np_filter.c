...
{
    /* Bug fix: if allocation fails after we've taken ownership of a and b,
     * we must free them to avoid a leak — the caller transferred ownership
     * and cannot recover them. */
    comb_priv_t *p = malloc(sizeof(*p));
    if (!p) { np_filter_free(a); np_filter_free(b); return NULL; }
    p->a = a; p->b = b;
    np_filter_t *f = calloc(1, sizeof(*f));
    if (!f) { free(p); np_filter_free(a); np_filter_free(b); return NULL; }
    f->ops = ops; f->priv = p;
    return f;
}
...
{
    /* Bug fix: same leak-on-failure as comb_new — if allocation fails
     * after taking ownership of a, free a to avoid a leak. */
    not_priv_t *p = malloc(sizeof(*p));
    if (!p) { np_filter_free(a); return NULL; }
    p->inner = a;
    np_filter_t *f = calloc(1, sizeof(*f));
    if (!f) { free(p); np_filter_free(a); return NULL; }
