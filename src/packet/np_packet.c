...
np_packet_t *np_packet_clone(const np_packet_t *src)
{
     ...
     * into pkt->raw).  The old code blindly computed
     *   off = src->layers[i].data - src->raw
     ...
     * Previously this was silently dropped, so a clone after TCP
     * reassembly or TLS decryption lost all payload. */
